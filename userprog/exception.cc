...
void Read_Char(){
	char c;
	c = SysReadChar();
	kernel->machine->WriteRegister(2, c);	// Ghi kí tự vào thanh ghi số 2
	UpPCRegister();
}
...
{
	int pa;
	pa = kernel->machine->ReadRegister(4);
	SysPrintNum(pa);
	UpPCRegister();
}
...
void Print_String()
{
	...
	// int length = kernel->machine->ReadRegister(5);
	// cerr << length << "\n";
    char* buffer = User2System(memPtr, MAX_LENGTH);
...
ExceptionHandler(ExceptionType which)
{
    ...
    switch (which) 
	{
		...
		case NumExceptionTypes:
			DEBUG(dbgSys, which << "Occuring exceptions\n");
